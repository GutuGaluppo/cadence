...
function sortTasks(tasks: Task[], activeTaskId: string | null): Task[] {
...
export function TaskListModal() {
  const isOpen = useTaskListModalStore((state) => state.isOpen);
  const close = useTaskListModalStore((state) => state.close);
  ...
  const sortedTasks = sortTasks(tasks, activeTaskId);
      ...
      <ModalPanel>
        ...
        {sortedTasks.length === 0 ? (
          <EmptyTasksState>
            ...
            <InlineAction onClick={handleOpenManagement} type="button">
              Manage tasks
            </InlineAction>
          </EmptyTasksState>
        ) : (
